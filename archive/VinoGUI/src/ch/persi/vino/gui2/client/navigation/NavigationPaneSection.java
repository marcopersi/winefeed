...
public class NavigationPaneSection extends SectionStackSection {
	...
	public NavigationPaneSection(String theSectionName,
			NavigationPaneRecord[] theSectionData, RecordClickHandler theClickHandler) {
		super(theSectionName);

		GWT.log("initialization of NavigationPaneSection runs", null);

		this.sectionData = theSectionData;

		// initialise the List Grid
		listGrid = new ListGrid();
		listGrid.setBaseStyle("vino-NavigationPaneGridCell");
		...
		listGrid.setData(theSectionData);
		listGrid.selectRecord(0);

		// register the click handler
		listGrid.addRecordClickHandler(theClickHandler);
	...
	public void selectRecord(String name) {
		for (int i = 0; i < this.sectionData.length; i++) {
			...
			if (name.contentEquals(record.getName())) {
				GWT.log("selectRecord->name.contentEquals(record.getName())",
						null);
	...
	public void setSectionData(NavigationPaneRecord[] theSectionData) {
		this.sectionData = theSectionData;
	}
