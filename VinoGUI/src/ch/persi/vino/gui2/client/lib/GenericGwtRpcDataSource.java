...
import ch.persi.vino.gui2.shared.GenericGwtRpcList;
...
public abstract class GenericGwtRpcDataSource<D, R extends Record, SA extends GenericGwtRpcServiceAsync<D>>
	...
	public GenericGwtRpcDataSource(String theId) {
		super();
		setID(theId);
		setDataSourceFields();
		serviceAsync = getServiceAsync();
	...
	protected void executeAdd(final String requestId, final DSRequest request,
		...
		serviceAsync.add(data, new AsyncCallback<D>() {
			...
			public void onSuccess(D result) {
				R aNewRecord = getNewRecordInstance();
				copyValues(result, aNewRecord);
				response.setData(new Record[] { aNewRecord });
				processResponse(requestId, response);
			}
		});
	}
